...
if (!defined('PDO::ATTR_DRIVER_NAME')) exit;
                    ...
                    if ($nInfo['status'] === 'online') {
                        ...
                        if (isset($nRrd['data']) && is_array($nRrd['data'])) {
                            for ($i = count($nRrd['data']) - 1; $i >= 0; $i--) {
                                if (isset($nRrd['data'][$i]['netin']) && $nRrd['data'][$i]['netin'] !== null) { $netin = $nRrd['data'][$i]['netin']; $netout = $nRrd['data'][$i]['netout']; break; }
                            }
                        }
        ...
        if (isset($vms['data'])) {
            foreach($vms['data'] as $vm) {
                ...
                if (!isset($seenVms[$uniqueId])) {
                    ...
                    if (isset($vm['status']) && $vm['status'] === 'running') $data['cluster_stats']['vms_running']++; else $data['cluster_stats']['vms_stopped']++;
                }
            }
        }
    }
        ...
        if (isset($vms['data'])) { 
            foreach ($vms['data'] as $vm) { 
                ...
                if (!isset($seenVms[$uniqueId]) && checkVmPermission($pdo, $vm['vmid'])) { 
                    $seenVms[$uniqueId] = true; $vm['node_id'] = $node['id']; $vm['node_ip'] = $node['ip_address']; $vm['host'] = $vm['node'] ?? 'unknown'; $allVms[] = $vm; 
                } 
            } 
        }
    }
...
// NEU: Node Dashboard Daten (VMs & Storage)
if ($action === 'get_node_dashboard') {
    if (!$isAdmin) exit;
    $stmt = $pdo->prepare("SELECT * FROM nodes WHERE id = ?"); $stmt->execute([$_GET['node_id']]); $node = $stmt->fetch(PDO::FETCH_ASSOC);
    $host = $_GET['host'];
    
    $vmsQemu = getProxmoxData($node['ip_address'], $node['token_id'], $node['token_secret'], "/api2/json/nodes/{$host}/qemu");
    $vmsLxc = getProxmoxData($node['ip_address'], $node['token_id'], $node['token_secret'], "/api2/json/nodes/{$host}/lxc");
    $storages = getProxmoxData($node['ip_address'], $node['token_id'], $node['token_secret'], "/api2/json/nodes/{$host}/storage");
    
    $nodeVms = [];
    if (isset($vmsQemu['data'])) { foreach ($vmsQemu['data'] as $v) { $v['type'] = 'qemu'; $v['host'] = $host; $v['node_id'] = $node['id']; $v['node_ip'] = $node['ip_address']; $nodeVms[] = $v; } }
    if (isset($vmsLxc['data'])) { foreach ($vmsLxc['data'] as $v) { $v['type'] = 'lxc'; $v['host'] = $host; $v['node_id'] = $node['id']; $v['node_ip'] = $node['ip_address']; $nodeVms[] = $v; } }
    usort($nodeVms, function($a, $b) { if ($a['status'] === 'running' && $b['status'] !== 'running') return -1; if ($a['status'] !== 'running' && $b['status'] === 'running') return 1; return $a['vmid'] <=> $b['vmid']; });
    
    echo json_encode(['success' => true, 'vms' => $nodeVms, 'storages' => $storages['data'] ?? []]); exit;
}
...
if ($action === 'add_node') { 
    if (!$isAdmin) exit; 
    $name = trim($_POST['name']); $ip = trim($_POST['ip']); $user = trim($_POST['user']); $pass = trim($_POST['pass']); $type = $_POST['type'] ?? 'pve';
    $tokenIdToSave = $user; $tokenSecretToSave = $pass;
    if ($type === 'pve') {
        $chAuth = curl_init("https://{$ip}:8006/api2/json/access/ticket");
        curl_setopt_array($chAuth, [ CURLOPT_RETURNTRANSFER => true, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_SSL_VERIFYHOST => false, CURLOPT_TIMEOUT => 5, CURLOPT_POST => true, CURLOPT_POSTFIELDS => http_build_query(['username' => $user, 'password' => $pass]) ]);
        $authRes = json_decode(curl_exec($chAuth), true); curl_close($chAuth);
        if(!isset($authRes['data']['ticket'])) { echo json_encode(['success' => false, 'error' => 'Proxmox Login fehlgeschlagen. Passwort falsch?']); exit; }
        $ticket = $authRes['data']['ticket']; $csrf = $authRes['data']['CSRFPreventionToken'];
        $tokenName = 'pvedash' . rand(1000, 9999);
        $chToken = curl_init("https://{$ip}:8006/api2/json/access/users/{$user}/token/{$tokenName}");
        curl_setopt_array($chToken, [ CURLOPT_RETURNTRANSFER => true, CURLOPT_SSL_VERIFYPEER => false, CURLOPT_SSL_VERIFYHOST => false, CURLOPT_TIMEOUT => 5, CURLOPT_POST => true, CURLOPT_POSTFIELDS => http_build_query(['privsep' => 0]), CURLOPT_HTTPHEADER => ["Cookie: PVEAuthCookie={$ticket}", "CSRFPreventionToken: {$csrf}"] ]);
        $tokenRes = json_decode(curl_exec($chToken), true); curl_close($chToken);
        if(!isset($tokenRes['data']['value'])) { echo json_encode(['success' => false, 'error' => 'Konnte API Token nicht erstellen. Admin-Rechte?']); exit; }
        $tokenIdToSave = $user . '!' . $tokenName; $tokenSecretToSave = $tokenRes['data']['value'];
    }
    $stmt = $pdo->prepare("INSERT INTO nodes (name, ip_address, token_id, token_secret, type) VALUES (?, ?, ?, ?, ?)"); $stmt->execute([$name, $ip, $tokenIdToSave, $tokenSecretToSave, $type]); 
...
if ($action === 'create_user') { 
    if (!$isAdmin) exit; 
    $stmt = $pdo->prepare("INSERT INTO users (username, password_hash, role, permissions) VALUES (?, ?, ?, ?)"); $stmt->execute([$_POST['username'], password_hash($_POST['password'], PASSWORD_DEFAULT), $_POST['role'], $_POST['permissions']]); 
...
// ANGEPASST: Update inkl. ISO und onboot
if ($action === 'update_vm_config') {
    ...
    if(isset($_POST['onboot'])) $params['onboot'] = $_POST['onboot'];
    if(isset($_POST['ide2'])) $params['ide2'] = $_POST['ide2'] === 'none' ? 'none,media=cdrom' : $_POST['ide2'] . ',media=cdrom';
...
if ($action === 'get_isos') {
    ...
    if (isset($stRes['data'])) {
        foreach ($stRes['data'] as $st) {
            if (strpos($st['content'], 'iso') !== false) {
                $cRes = getProxmoxData($node['ip_address'], $node['token_id'], $node['token_secret'], "/api2/json/nodes/{$_GET['host']}/storage/{$st['storage']}/content");
                if (isset($cRes['data'])) {
                    foreach ($cRes['data'] as $item) {
                        if ($item['content'] === 'iso') $isos[] = $item['volid'];
                    }
                }
            }
        }
    }
    echo json_encode(['success' => true, 'data' => $isos]); exit;
}

// NEU: Node-Spezifische Backup Jobs
if ($action === 'get_node_backup_jobs') {
    if (!$isAdmin) exit;
    $stmt = $pdo->prepare("SELECT * FROM nodes WHERE id = ?"); $stmt->execute([$_GET['node_id']]); $node = $stmt->fetch(PDO::FETCH_ASSOC);
    $res = getProxmoxData($node['ip_address'], $node['token_id'], $node['token_secret'], "/api2/json/cluster/backup");
    $hostJobs = [];
    if (isset($res['data'])) {
        foreach ($res['data'] as $job) {
            // Zeige nur Jobs, die diesen Host betreffen (oder alle, falls node nicht spezifiziert in PVE)
            if (empty($job['node']) || $job['node'] === $_GET['host']) {
                $hostJobs[] = $job;
            }
        }
    }
    echo json_encode(['success' => true, 'data' => $hostJobs]); exit;
}
    ...
    if ($action === 'get_node_status') {
        ...
        if (isset($rrd['data']) && is_array($rrd['data'])) {
            for ($i = count($rrd['data']) - 1; $i >= 0; $i--) {
                if (isset($rrd['data'][$i]['netin']) && $rrd['data'][$i]['netin'] !== null) { $netin = $rrd['data'][$i]['netin']; $netout = $rrd['data'][$i]['netout']; break; }
            }
        }
        $data = $res['data'] ?? []; $data['netin'] = $netin; $data['netout'] = $netout; $data['is_rrd_net'] = true;
        echo json_encode(['success' => true, 'data' => $data]); exit;
    } else {
...
if ($action === 'get_historical_rrd') {
    if (!checkVmPermission($pdo, $_GET['vmid'] ?? 0)) exit;
    $stmt = $pdo->prepare("SELECT * FROM nodes WHERE id = ?"); $stmt->execute([$_GET['node_id']]); $node = $stmt->fetch(PDO::FETCH_ASSOC);
    $timeframe = $_GET['timeframe'] === 'week' ? 'week' : 'day'; $type = $node['type'] ?? 'pve';
    ...
    } else { $endpoint = "/api2/json/nodes/{$_GET['host']}/{$_GET['target_mode']}/{$_GET['vmid']}/rrddata?timeframe={$timeframe}&cf=AVERAGE"; }
    $res = getProxmoxData($node['ip_address'], $node['token_id'], $node['token_secret'], $endpoint, $type);
    echo json_encode(['success' => true, 'data' => $res['data'] ?? []]); exit;
}
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $node) {
        ...
        if (isset($nData['data'])) {
            foreach ($nData['data'] as $nInfo) {
                ...
                if (isset($apt['data']) && count($apt['data']) > 0) { $updateList[] = [ 'node_id' => $node['id'], 'host' => $internalName, 'display_name' => $node['name'] . ' (' . $internalName . ')', 'type' => $node['type'], 'packages' => $apt['data'] ]; }
            }
        }
    }
